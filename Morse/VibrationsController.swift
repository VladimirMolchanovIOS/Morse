...
class VibrationsController: NSObject {
    ...
    private var _currentMarkIndex: String.CharacterView.Index!
    ...
    func transmitMessage(message: String) {
        ...
        _currentMarkIndex = _message.startIndex
    ...
    func manageVibrations() {
        ...
        if _currentMarkIndex == _message.startIndex {
            print("manageVibrations: 1st condition")
            if _message[_currentMarkIndex] == "." {
                print("manageVibrations: 1st+ condition")
                AudioServicesPlaySystemSound(1033)
            }
        } else {
            print("manageVibrations: 2nd condition")
            if _message[_currentMarkIndex] == "." && _message[_currentMarkIndex.predecessor()] != "." {
                print("manageVibrations: 2nd+ condition")
                AudioServicesPlaySystemSound(1033)
            }
        }

        if _currentMarkIndex == _message.characters.indices.last! {
